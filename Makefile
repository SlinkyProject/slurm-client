...
generate-api-matrix: ## Generate Slurm OpenAPI spec files by matrix.
	declare -A VERSION_MATRIX=( \
		["ghcr.io/slinkyproject/slurmrestd:25.11.0-ubuntu24.04"]="" \
...
THIRD_PARTY_LICENSES: $(GO_LICENSES)
	...
	{ \
		...
		printf 'This file contains license information for third-party software components used in slurm-client.\n\n'; \
		...
		printf '## Dependencies\n\n'; \
		first=true; \
		$(GO_LICENSES) report "$(LICENSE_PACKAGE_PATTERN)" --template "$$tpl" --logtostderr=false --log_file=/dev/null --stderrthreshold=FATAL | LC_ALL=C sort -u | \
