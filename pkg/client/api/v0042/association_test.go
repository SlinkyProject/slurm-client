...
func TestSlurmClient_CreateAssoc(t *testing.T) {
	tests := []struct {
		...
		{name: "default", req: api.V0042Assoc{User: "alice", Account: ptr.To("research")}, wantErr: false},
...
func TestSlurmClient_DeleteAssoc(t *testing.T) {
	c := &SlurmClient{ClientWithResponsesInterface: fake.NewFakeClient()}
	err := c.DeleteAssoc(context.Background(), api.V0042Assoc{User: "alice", Account: ptr.To("research")})
	if err != nil {
