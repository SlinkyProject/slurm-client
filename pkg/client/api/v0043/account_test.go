			...
			client: fake.NewFakeClientBuilder().
				WithInterceptorFuncs(interceptor.Funcs{
					SlurmdbV0043GetAccountWithResponse: func(ctx context.Context, accountName string, params *api.SlurmdbV0043GetAccountParams, reqEditors ...api.RequestEditorFn) (*api.SlurmdbV0043GetAccountResponse, error) {
						return &api.SlurmdbV0043GetAccountResponse{
							HTTPResponse: &fake.HttpSuccess,
							JSON200: &api.V0043OpenapiAccountsResp{
								Accounts: api.V0043AccountList{{Name: "research"}},
							},
						}, nil
					},
				}).
				Build(),
			want:    &types.V0043Account{V0043Account: api.V0043Account{Name: "research"}},
			wantErr: false,
		},
		{
				...
				WithInterceptorFuncs(interceptor.Funcs{
					SlurmdbV0043GetAccountWithResponse: func(ctx context.Context, accountName string, params *api.SlurmdbV0043GetAccountParams, reqEditors ...api.RequestEditorFn) (*api.SlurmdbV0043GetAccountResponse, error) {
						return &api.SlurmdbV0043GetAccountResponse{
							...
							JSONDefault: &api.V0043OpenapiAccountsResp{
								Errors: &[]api.V0043OpenapiError{{Error: ptr.To("error 1")}},
							},
						}, nil
					},
				}).
...
func TestSlurmClient_ListAccount(t *testing.T) {
	tests := []struct {
		name    string
		client  api.ClientWithResponsesInterface
		want    *types.V0043AccountList
		wantErr bool
	}{
		{
			name:    "Empty list",
			client:  fake.NewFakeClient(),
			want:    &types.V0043AccountList{Items: make([]types.V0043Account, 0)},
			wantErr: false,
		},
		{
			name: "Non-empty list",
			client: fake.NewFakeClientBuilder().
				WithInterceptorFuncs(interceptor.Funcs{
					SlurmdbV0043GetAccountsWithResponse: func(ctx context.Context, params *api.SlurmdbV0043GetAccountsParams, reqEditors ...api.RequestEditorFn) (*api.SlurmdbV0043GetAccountsResponse, error) {
						return &api.SlurmdbV0043GetAccountsResponse{
							HTTPResponse: &fake.HttpSuccess,
							JSON200: &api.V0043OpenapiAccountsResp{
								Accounts: api.V0043AccountList{{Name: "a"}, {Name: "b"}},
							},
						}, nil
					},
				}).
				Build(),
			want: &types.V0043AccountList{Items: []types.V0043Account{
...
func TestSlurmClient_CreateAccount(t *testing.T) {
	tests := []struct {
		...
		{name: "default", req: api.V0043Account{Name: "research"}, want: "research", wantErr: false},
		{name: "invalid type provided", req: api.V0043User{Name: "research"}, wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
