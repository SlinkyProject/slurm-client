...
func parseNode(nodeV api.V0041Node) slurmtypes.Node {
	node := slurmtypes.Node{
		...
		State:         make(set.Set[slurmtypes.NodeState], 0),
	...
	for _, state := range states {
		node.State.Insert(slurmtypes.NodeState(state))
	}
	return node
}
...
func (c *SlurmClient) GetNode(ctx context.Context, nodeName string) (*slurmtypes.Node, error) {
...
func (c *SlurmClient) ListNodes(ctx context.Context) (*slurmtypes.NodeList, error) {
...
func (c *SlurmClient) UpdateNode(ctx context.Context, node *slurmtypes.Node, originalNode *slurmtypes.Node) error {
