...
type AssocInterface interface {
	...
	DeleteAssoc(ctx context.Context, assoc api.V0044Assoc) error
	GetAssoc(ctx context.Context, key string) (*types.V0044Assoc, error)
	ListAssoc(ctx context.Context) (*types.V0044AssocList, error)
}
...
func (c *SlurmClient) CreateAssoc(ctx context.Context, req any) (string, error) {
	r, ok := req.(api.V0044Assoc)
	if !ok {
		return "", errors.New("expected req to be api.V0044Assoc")
	}
	body := api.SlurmdbV0044PostAssociationsJSONRequestBody{
		Associations: api.V0044AssocList{r},
	}
	res, err := c.SlurmdbV0044PostAssociationsWithResponse(ctx, body)
	if err != nil {
		return "", err
	}
	...
	assoc := &types.V0044Assoc{V0044Assoc: r}
...
func (c *SlurmClient) DeleteAssoc(ctx context.Context, assoc api.V0044Assoc) error {
	params := &api.SlurmdbV0044DeleteAssociationParams{
		Account:   assoc.Account,
		User:      nonEmptyPtr(assoc.User),
		Cluster:   assoc.Cluster,
		Partition: assoc.Partition,
	}
	res, err := c.SlurmdbV0044DeleteAssociationWithResponse(ctx, params)
	if err != nil {
		return err
	}
	if res.StatusCode() != 200 {
...
// every association.
func (c *SlurmClient) GetAssoc(ctx context.Context, key string) (*types.V0044Assoc, error) {
	cluster, account, user, partition := parseAssocKey(key)
	...
	res, err := c.SlurmdbV0044GetAssociationsWithResponse(ctx, params)
	if err != nil {
		return nil, err
	}
	if res.StatusCode() != 200 {
...
func (c *SlurmClient) ListAssoc(ctx context.Context) (*types.V0044AssocList, error) {
	params := &api.SlurmdbV0044GetAssociationsParams{}
	res, err := c.SlurmdbV0044GetAssociationsWithResponse(ctx, params)
	if err != nil {
		return nil, err
	...
	list := &types.V0044AssocList{
		Items: make([]types.V0044Assoc, len(res.JSON200.Associations)),
	}
